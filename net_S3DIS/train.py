...
def train(args, graph_inf, net_inf):
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(args.gpu)):
            pointclouds_pl, labels_pl, is_training_pl = placeholder_inputs(feature_channel, with_spw=False)
        ...
        for epoch in range(args.max_epoch):
            ...
            train_one_epoch(args, sess, ops, train_writer, train_data, train_label, LOG_FOUT)
  
            # Save the variables to disk.
            if epoch % 3 == 0:
                iou = eval_one_epoch(args, sess, ops, test_writer, test_data, test_label, LOG_FOUT)
                if iou>best_iou:
                    best_iou = iou
...
def train_one_epoch(args, sess, ops, train_writer, train_data, train_label, LOG_FOUT):
...
def eval_one_epoch(args, sess, ops, test_writer, test_data, test_label, LOG_FOUT):
    """ ops: dict mapping from string to tf ops """
    is_training = False
    total_correct = 0
    total_seen = 0
    ...
    for batch_idx in range(num_batches):
        ...
        gt_classes, positive_classes, true_positive_classes = provider.utils_iou(
                    gt_classes, positive_classes, true_positive_classes, pred_val, batch_label)
    iou_list = []
...
def load_traindata(data_path):
    ...
    TEST_FILES = glob.glob(test_path + '/*.h5')
